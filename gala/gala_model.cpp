...
namespace gala {
	...
	GalaModel::~GalaModel() {
		if (hasIndexBuffer) {
		vkDestroyBuffer(galaDevice.device(), indexBuffer, nullptr);
		vkFreeMemory(galaDevice.device(), indexBufferMemory, nullptr);
		}
	}
	...
	void GalaModel::createVertexBuffers(const std::vector<Vertex>& vertices) {
		...
		void* data;
		vkMapMemory(galaDevice.device(), stagingBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
		vkUnmapMemory(galaDevice.device(), stagingBufferMemory);
		...
		galaDevice.copyBuffer(stagingBuffer, vertexBuffer, bufferSize);

		vkDestroyBuffer(galaDevice.device(), stagingBuffer, nullptr);
		vkFreeMemory(galaDevice.device(), stagingBufferMemory, nullptr);
	}
	...
	void GalaModel::createIndexBuffers(const std::vector<uint32_t>& indices) {
		...
		galaDevice.copyBuffer(stagingBuffer, indexBuffer, bufferSize);
